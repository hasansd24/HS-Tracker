...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  ...
  .day-label-wrap:active { background: var(--glass-bg); }
  ...
  .day-sub.not-today::after {
    content: ' · tap for today';
...
<div class="app">
  ...
  <div class="day-nav">
    ...
    <div class="day-label-wrap" id="dayLabelWrap">
...
// Qatar timezone helper (UTC+3, no DST)
function qatarTodayISO() {
  const now = new Date();
  // Convert to Qatar time by adjusting UTC offset
  const qatarMs = now.getTime() + (now.getTimezoneOffset() * 60000) + (3 * 3600000);
  const qatar = new Date(qatarMs);
  return qatar.toISOString().slice(0, 10);
}

let state = { days: {}, currentDate: qatarTodayISO(), view: 'today', settings: { theme: 'ocean', apiKey: '', targets: null } };
...
function loadState() {
  try {
    ...
    // Always start on today in Qatar time when the app opens
    state.currentDate = qatarTodayISO();
...
function fmtSub(iso) {
  const todayIso = qatarTodayISO();
  if (iso === todayIso) return 'Today';
  // Compute yesterday/tomorrow in Qatar time
...
function nicotineStreak() {
  ...
  const todayQatar = isoToDate(qatarTodayISO());
  return Math.max(0, Math.floor((todayQatar - quit) / 86400000) + 1);
}
...
function render() {
  ...
  const subLabel = fmtSub(state.currentDate);
  const subEl = document.getElementById('daySub');
  subEl.textContent = subLabel;
  // Highlight when not on today's date so user knows
  subEl.classList.toggle('not-today', subLabel !== 'Today');
...
document.getElementById('dayLabelWrap').addEventListener('click', jumpToToday);
...
// Auto-rollover: when the app is reopened or comes back to focus and a new day has started in Qatar, jump to it
let lastSeenQatarDate = qatarTodayISO();

function checkDayRollover() {
  const nowQatar = qatarTodayISO();
  if (nowQatar !== lastSeenQatarDate) {
    lastSeenQatarDate = nowQatar;
    // If the user was on the previous "today", advance to the new today automatically
    // If they were browsing past dates, leave them alone but refresh streak/labels
    const wasOnToday = state.currentDate === lastSeenQatarDate || state.days[state.currentDate]?.__wasToday;
    if (wasOnToday || state.view === 'today') {
      state.currentDate = nowQatar;
      if (!state.days[nowQatar]) state.days[nowQatar] = blankDay();
      saveState();
    }
    render();
  } else {
    // Same day, just refresh streak counter (in case it ticked)
    document.getElementById('streakNum').textContent = nicotineStreak();
  }
}

// Check when the tab regains focus (most common case on iPhone)
document.addEventListener('visibilitychange', () => {
  if (!document.hidden) checkDayRollover();
});
window.addEventListener('focus', checkDayRollover);

// Also check every 60 seconds while open
